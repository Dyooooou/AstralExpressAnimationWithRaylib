...
void drawMekanisme(float timer) {
    ...
    if (mekanismeSlide == 0) {
        ...
        DrawText(TextFormat("sx = 1 + %.2f*3 = %.2f", wf, 1.0f + wf*3.0f), (int)(kx - stretchHalf), arrY + 10, 16, (Color){255,200,50,255});
 
        // ── Panel Kanan: Bintang Streak ───────────────────────────
        ...
        for (int i = 0; i < 12; i++) {
            ...
            if (wf > 0.2f) {
                ...
                int thick = (int)(sz * 0.7f); if (thick < 1) thick = 1;
        ...
        DrawText(TextFormat("Live  >>  warpFactor = %.3f   sx = %.3f   sy = %.3f", wf, 1.0f + wf*3.0f, 1.0f - wf*0.3f), 70, SH-145, 16, (Color){255,220,80,255});
        ...
        DrawText(TextFormat("Live  >>  streak contoh = %.1f px", wf * 40.0f * 0.3f), SW-740, SH-145, 16, (Color){255,220,80,255});
    }
 
    // SLIDE 1: KINEMATIKA RODA
        ...
        int cx = 350, cy = 700, R = 80, cr = 30;
        DrawText("Detail: Crank-Slider (Roda + Rod + Piston)", 60, 590, 18, WHITE);
        MidcircleFilled(cx, cy, R, (Color){180,185,195,255});
        MidcircleFilled(cx, cy, (int)(R*0.7f), (Color){40,40,45,255});
        for (int j = 0; j < 8; j++) {
            float a = theta + j * (2.0f*PI/8.0f);
            int px = cx + (int)((R*0.8f)*cosf(a));
            int py = cy + (int)((R*0.8f)*sinf(a));
            Bres_ThickLine(cx, cy, px, py, 2, (Color){180,185,195,255});
        }
        MidcircleFilled(cx, cy, (int)(R*0.3f), (Color){180,185,195,255});
        MidcircleFilled(cx, cy, (int)(R*0.15f), (Color){0,0,0,255});
 
        int pinX = cx + (int)(cr * cosf(theta)), pinY = cy + (int)(cr * sinf(theta));
        MidcircleFilled(pinX, pinY, 8, (Color){255,100,100,255});
 
        int pistonX = cx + 200;
        Bres_ThickLine(cx, cy, pinX, pinY, 3, (Color){255,100,100,200}); 
        Bres_ThickLine(pinX, pinY, pistonX, pinY, 3, (Color){180,185,195,200}); 
        
        // Piston digambar dengan Algoritma Rectangle Filled
        ...
        DrawText(TextFormat("Pin X = %d + %d*cos(%.1f) = %d", cx, cr, theta, pinX), 480, 640, 16, (Color){255,100,100,255});
        DrawText(TextFormat("Pin Y = %d + %d*sin(%.1f) = %d", cy, cr, theta, pinY), 480, 665, 16, (Color){255,100,100,255});
        DrawText(TextFormat("Piston X = Pin X + panjang_rod"), 480, 695, 16, (Color){180,220,255,255});
        ...
        DrawText(TextFormat("Live  >>  theta = %.2f rad   omega = %.2f rad/s", theta, 3.5f), SW-760, SH-108, 16, (Color){255,220,80,255});
    }
    ...
    // SLIDE 2: PORTAL WARP
        ...
        for (int i = 0; i < 6; i++) {
            ...
            int px = hcx + (int)(vx*cosD - vy*sinD), py = hcy + (int)(vx*sinD + vy*cosD);
        ...
        DrawText(TextFormat("Live  >>  rot = %.2f rad   openL = %.2f   openR = %.2f", rotDiag, openL, openR), 60, SH-135, 15, (Color){255,220,80,255});
