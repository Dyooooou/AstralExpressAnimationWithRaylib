...
void drawBintang(float warpFactor, Vector2 offset) {
    for (int i = 0; i < NUM_STARS; i++) {
        ...
        if (showOutline) {
            ...
            // Menggunakan threshold warp 0.05f agar efek garis lebih cepat merespons
            if (warpFactor > 0.05f) {
                // Kalkulasi panjang garis eksponensial (memenuhi layar saat warp maksimal)
                ...
                // Karena bintang bergerak ke kiri, buntut cahaya (streak) membentang ke arah kanan (+ streak)
                int endX = (int)(cx + streak); 
                
                // Efek visual: warna bintang memutih terang saat warp sangat cepat
                Color starColor = (Color){200, 220, 255, 200};
                if (warpFactor > 0.8f) {
                    starColor = (Color){255, 255, 255, 255}; 
                }

                // Gambar garis lightspeed yang sudah terkena efek camera shake
                Bres_ThickLine(
                    (int)cx, (int)cy,
                    endX, (int)cy,
                    thick, starColor
                ...
                // Gambar bintang diam yang juga terpengaruh camera shake
                MidcircleFilled(
                    (int)cx, (int)cy,
...
//         } else {
//             // Jika ada pergerakan / warp, ubah titik menjadi garis (Lightspeed effect)
//             if (warpFactor > 0.05f) {
//                 // Streak dihitung eksponensial. Semakin tinggi warp, multiplier panjangnya makin gila-gilaan (memenuhi layar)
//                 float streakMultiplier = 10.0f + (warpFactor * warpFactor * 300.0f);
//                 float streak = warpFactor * bintang[i].speed * streakMultiplier;
...
//                 int endX = (int)(bintang[i].x + streak); // Gambar garis ke arah belakang bintang
                
//                 // Warnanya dibikin sedikit lebih cerah/putih saat warp tinggi
//                 Color starColor = (Color){200, 220, 255, 200};
//                 if (warpFactor > 0.8f) {
//                     starColor = (Color){255, 255, 255, 255}; 
//                 }
...
//                     endX, (int)bintang[i].y,
//                     thick, starColor
//                 );
//             } else {
//                 // Jika diam atau warp sangat kecil, gambar sebagai lingkaran biasa
//                 MidcircleFilled(
//                     (int)bintang[i].x, (int)bintang[i].y,
//                     (int)bintang[i].size, (Color){200, 220, 255, 180}
//                 );
